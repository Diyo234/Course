...
def course(lesson_name):
    ...
    if "%image%" in html_content:
        image_file = extract_between_strings(html_content, "%image%", "%end image%")
        html_content = replace_between_strings(html_content, "%image%", "%end image%", f"<img src='/static/Images/{image_file}' alt=''>")
    if "%flashcard block%" in html_content:
        flashcard_block_content = extract_between_strings(html_content, "%flashcard block%", "%end flashcard block%")
        ...
        """
        html_content = replace_between_strings(html_content, "%flashcard block%", "%end flashcard block%", flashcard_block_html)
    if "%multi-choice%" in html_content:
        multi_choice_block_content = extract_between_strings(html_content, "%multi-choice%", "%end multi-choice%")
        ...
        """
        html_content = replace_between_strings(html_content, "%multi-choice%", "%end multi-choice%", multi_choice_html)
    if "%gap-fill%" in html_content:
        gap_fill_content = extract_between_strings(html_content, "%gap-fill%", "%end gap-fill%")
        ...
        """
        html_content = replace_between_strings(html_content, "%gap-fill", "%end gap-fill%", gap_fill_html)
    if "single-answer" in html_content:
        single_answer_content = extract_between_strings(html_content, "%question%", "%end single-answer%")
        ...
        """
        html_content = replace_between_strings(html_content, "%single-answer%", "%end single-answer%", single_answer_html)
